...
import 'package:ai_school/features/assessments/providers/evolution_provider.dart';
...
class EvolutionPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final sections = [...assessment.sections]..sort((a, b) => a.index.compareTo(b.index));
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: PreferredSize(
          preferredSize: Size.fromHeight(32 + (assessment.activity != null? 32: 0)),
          child: Column(
            children: [
              StudentHeader(student: args.student, class$: args.class$),
              if (assessment.activity != null)
        ...
        data: (initial) {
          return HookBuilder(
            builder: (context) {
              ...
              final edit = evolution.value.id.isNotEmpty;

              Future<void> saveEvolution() async {
                if (!formKey.currentState!.validate()) return;
                loading.value = true;
                try {
                  final repo = ref.read(evolutionRepositoryProvider);

                  var updated = evolution.value;

                  if (updated.id.isEmpty) {
                    updated = await repo.createEvolution(updated);
                  } else {
                    updated = await repo.updateEvolution(updated.id, updated);
                  }
                          ...
                          child: Column(
                            ...
                            children:
                                sections.map((section) {
                                  final elements = [...section.elements]..sort(
